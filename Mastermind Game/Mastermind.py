...
'''
# temp cut feb 18, 2022 @ 10:10pm

modified = []
count = 0
for a in fromCSV:
    modified.append(''.join(a))
    count += 1

print("sucessfully printed: \n", modified)       # had to make Handler.readIn() return an array of arrays containing the data from the csv file

print("sucessfully printed: \n", fromCSV[0])    # proper way to handle this is to access the line via indicies []
print("sucessfully printed: \n", fromCSV[0][1])
'''
GameOver = False

maxInput = 4
numericalRange = 6  # the largest value for a peg usable, with the smallest being 1
totalTries = 8
currentTry = 1
...
# vvvvvvvvvvv HANDLERV1_1 TESTING vvvvvvvvvvv
